...
const CLASSES: {
  ...
  { id: 'economy',         name: 'Economy',         tagline: 'Get there affordably',   seat: 'Standard seat', pitch: '30–32" pitch',       image: 'https://images.unsplash.com/photo-1569094648429-de3e73df0e25?w=600&q=70', accent: '#6B7280', multiplier: '1×',   perks: ['Carry-on + checked bag', 'In-flight entertainment', 'Meal on long-haul'],                               airlines: 'PIA · FlyDubai · Air Arabia' },
  ...
  { id: 'business',        name: 'Business',        tagline: 'Arrive feeling ready',    seat: 'Lie-flat',       pitch: '180° flat bed',      image: 'https://images.unsplash.com/photo-1529680977620-6c88f7db0ee7?w=600&q=70', accent: '#8B5CF6', multiplier: '2.2×', perks: ['Lounge access', 'Gourmet dining', 'Chauffeur service', 'Priority everywhere', 'Amenity kit'], popular: true, airlines: 'Emirates · Qatar Qsuite' },
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {CLASSES.map((cls, i) => {
          ...
          return (
            ...
            >
              <button
                onClick={() => { setFlightClass(cls.id); setTimeout(() => nextStep(), 220); }}
                className={cn(
              ...
              >
                ...
                <div className="relative overflow-hidden" style={{ height: '170px' }}>
                  ...
                  {cls.popular && (
                    <div className="absolute top-3.5 left-3.5 text-[11px] font-bold px-3 py-1 rounded-full"
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-4">
                    <div className="flex items-end justify-between">
                      <div>
                        <p className="text-[11px] font-semibold uppercase tracking-widest mb-1" style={{ color: `${cls.accent}CC` }}>
                          {cls.seat} · {cls.pitch}
                        </p>
                        <h3 className="text-[1.35rem] font-bold text-white leading-tight">{cls.name}</h3>
                        <p className="text-[12px] text-white/50 mt-0.5">{cls.tagline}</p>
                      </div>
                      <div className="text-[14px] font-bold px-3 py-1.5 rounded-xl shrink-0 ml-3"
                ...
                <div className="p-4 bg-[var(--navy-card)]">
                  <ul className="space-y-2">
                    {cls.perks.map((p) => (
                      <li key={p} className="flex items-center gap-2.5 text-[13px] text-[var(--text-secondary)]">
                        <Check className="w-3.5 h-3.5 shrink-0" style={{ color: cls.accent }} strokeWidth={2.5} />
                        {p}
                      </li>
                    ))}
                  </ul>
                  <div className="mt-3 pt-3 border-t border-[var(--border)] flex items-center gap-1.5 text-[11px] text-[var(--text-muted)]">
